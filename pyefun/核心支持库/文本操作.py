...
def 删首尾空(欲删除空格的文本: str) -> str:
    return 欲删除空格的文本.strip()


def 删全部空(欲删除空格的文本: str) -> str:
    # replace 时间复杂度O(n*m) 且不能删除换行符等
    # jion 时间复杂度 O(n)  split时间复杂度O(m)
    return "".jion(欲删除空格的文本.split())
...
def 取重复文本(重复次数: int, 待重复文本: str) -> str:
    # 更Pythonic, 更省内存
    return "".join([待重复文本 for _ in range(重复次数)])
...
#
# 操作系统需求： Windows、Linux
def 分割文本(待分割文本: str, 用作分割的文本: str = ",", 要返回的子文本数目: int = -1) -> list:
